...
class T1DPatient(Patient):
    ...
    def step(self, action, scenario_action, uhr, ex_start, ex_end, beta, to_record_model_eq=False, testing_GI_limits=False):
        ...
        self._add_meal_to_queue(scenario_action.CHO, round(scenario_action.kGI, ndigits=2))
        to_eat, kGI = self._announce_meal(action.CHO, round(action.kGI, ndigits=2))
    ...
    def model(t, x, action, params, last_Qsto, last_foodtaken, PAparams, beta, uhr, uhr_end, t_start, t_end, to_record_model_eq=False, testing_GI_limits=False):
        dxdt = np.zeros(13 + 2 + (100+1)*3)  # + dxdt[-2] = dydx, dxdt[-1] = dzdt   + (10+1) (dxdt[0], dxdt[1], dxdt[2])
        ...
        # ###################### External insulin ######################################
        # # Rate of appearance of external insulin:
        # Rait = params.ka1 * x[10] + params.ka2 * x[11]          # RaI(t) = ka1 * Isc1(t) + ka2 * Isc2(t)
        
        
        ###################### Insulin secretion ######################################
        # Insulin secration from beta-cells:
        St = params.gamma * x[10]   # S(t) = gamma * Ipo(t)
        
        
        ###############################################################################
        # insulin kinetics
        dxdt[5] = ( # dIp/dt = -(m2 + m4) * Ip(t) + m1 * Il(t) (+ Rai(t) if T1DM) (+ S(t) if healthy)
            -(params.m2 + params.m4) * x[5] + params.m1 * x[9] + St # + Rait
        )  # plus insulin IV injection u[3] if needed
        
        It = x[5] / params.Vi                               # I(t) = Ip(t)/Vi,          I(0) = Ib
        dxdt[5] = (x[5] >= 0) * dxdt[5]
        
        # insulin action on glucose utilization
        dxdt[6] = -params.p2u * x[6] + params.p2u * (It - params.Ib)    # dX/dt = -p2u * X(t) + p2u * [I(t) - Ib],          X(0) = 0
        
        # insulin action on production
        dxdt[7] = -params.ki * (x[7] - It)      # dI'/dt = - ki * [I'(t) - I(t)],            I'(0) = Ib
        
        dxdt[8] = -params.ki * (x[8] - x[7])    # dX^L/dt = - ki * [X^L(t) - I'(t)],            X^L(0) = Ib
        
        # insulin in the liver (pmol/kg)
        dxdt[9] = -(params.m1 + params.m30) * x[9] + params.m2 * x[5]   # dIl/dt = -(m1 + m3) * Il(t) + m2 * Ip(t),         Il(0) = Ilb
        dxdt[9] = (x[9] >= 0) * dxdt[9]
        
        # ######################## Subcutaneous insulin kinetics ########################
        # dxdt[10] = insulin - (params.ka1 + params.kd) * x[10]   # dIsc1/dt = -(kd + ka1) * Isc1(t) + IIR(t),        Isc1(0) = Isc1ss
        # dxdt[10] = (x[10] >= 0) * dxdt[10]
        
        # dxdt[11] = params.kd * x[10] - params.ka2 * x[11]       # dIsc2/dt = kd * Isc1(t) - ka2 * Isc2(t),          Isc2(0) = Isc2ss
        # dxdt[11] = (x[11] >= 0) * dxdt[11]
        
        
        ###################### Insulin secretion kinetics #############################
        if dxdt[3] > 0:
            Spo = x[11] + params.K * dxdt[3] + params.Sb
        else:
            Spo = x[11] + params.Sb
        
        # dIpo/dt
        dxdt[10] = -params.gamma * x[10] + Spo
        
        # dYpo/dt
        if params.beta * (x[3] - params.Gpb) >= -params.Sb:      # h = Gb to guarantee system steady state in basal conditions
            dxdt[11] = -params.alpha * (x[11] - params.beta * (x[3] - params.Gpb))
        else:
            dxdt[11] = -params.alpha * x[11] - params.alpha * params.Sb
        
        ###############################################################################
        
        # subcutaneous glucose
        dxdt[12] = -params.ksc * x[12] + params.ksc * x[3]          # dGs/dt = -1/Ts * Gs(t) + 1/Ts * Gp(t)         Gs(0) = Gpb = Vg*Gb
    ...
    def reset(self):
        ...
        new_init_state = np.zeros(len(self.init_state)+2+3*(100+1))
