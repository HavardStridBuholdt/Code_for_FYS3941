...
class Viewer(object):
    def __init__(self, start_time, patient_name, ex_start, ex_end, figsize=None):
        self.start_time = start_time
        self.patient_name = patient_name
        self.ex_start = self.start_time + timedelta(minutes=int(ex_start))
        self.ex_end = self.start_time + timedelta(minutes=int(ex_end))
    ...
    def initialize(self):
        ...
        axes[2].set_ylabel('uhr (bpm)')
        axes[3].set_ylabel('Insulin (U/min)')
        axes[4].set_ylabel('Risk Index')

        lineBG, = axes[0].plot([], [], label='BG')
        lineCGM, = axes[0].plot([], [], label='CGM')
        lineCHO, = axes[1].plot([], [], label='CHO')
        lineUhr, = axes[2].plot([], [], label='uhr')
        lineIns, = axes[3].plot([], [], label='Insulin')
        lineLBGI, = axes[4].plot([], [], label='Hypo Risk')
        lineHBGI, = axes[4].plot([], [], label='Hyper Risk')
        lineRI, = axes[4].plot([], [], label='Risk Index')

        lines = [lineBG, lineCGM, lineCHO, lineUhr, lineIns, lineLBGI, lineHBGI, lineRI]
        ...
        for ax in axes:
            ...
            ax.legend()
            ax.axvspan(self.ex_start, self.ex_end, alpha=0.2, color="gray", lw=0)
