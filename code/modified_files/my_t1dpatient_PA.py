...
class T1DPatient(Patient):
    ...
    def step(self, action, scenario_action, uhr, ex_start, ex_end, beta, to_record_model_eq=False, testing_GI_limits=False):
        ...
        self._add_meal_to_queue(scenario_action.CHO, scenario_action.kGI)
        to_eat, kGI = self._announce_meal(0, 0)
        if self._last_kGI > 0 and kGI <= 0:
            kGI = self._last_kGI
        ...
        if self._odesolver.successful():
            ...
            raise(f"ODE solver failed. \naction: {action.CHO, action.kGI}, \nscenario_act: {scenario_action.CHO, scenario_action.kGI}, \nuhr: {uhr}, \nexercise session: {ex_start, ex_end}")
    ...
    def model(t, x, action, params, last_Qsto, last_foodtaken, PAparams, beta, uhr, uhr_end, t_start, t_end, to_record_model_eq=False, testing_GI_limits=False):
        ...
        # the glucose rate of appearance constant (Rat) independently for 1
        ...
        dxdt[0:3], Rat = digest(x[0:3], action.kGI, d, kgut, params)
        ####################################################################
