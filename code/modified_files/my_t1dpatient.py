...
class W_recorder():
    def __init__(self):
        """A class to record the estimate the area under the curve of (HR(t) - HR) ie. the w parameter"""
        self.w = 0
        self.has_started = False
        

    def update(self, t, HR, HRb, start, end, tz):
        ...
        if start <= t:
            if (t-end) < tz:
                # print(f"last t = {self.last_t}, t = {t} delta t = {t - self.last_t}")
                delta_t = t - self.last_t
                self.w += (HR - HRb)*delta_t
            self.last_t = t
    
    def __call__(self):
        return self.w

W = W_recorder()        # A global class, allowing the w parameter to be estimated continuously trough the integration process of the models diff equations
...
class T1DPatient(Patient):
    ...
    def step(self, action, HR, HRb, ex_start, ex_end):
        ...
        self._odesolver.set_f_params(
            action, self._params, self._last_Qsto, self._last_foodtaken, self.breton_parms, HR, HRb, ex_start, ex_end
        )
    ...
    @staticmethod
    def model(t, x, action, params, last_Qsto, last_foodtaken, bparams, HR, HRb, t_start, t_end):
        dxdt = np.zeros(13 + 2)  # dxdt[-2] = dydx  &  dxdt[-1] = dzdt
        ...
        Kmt = params.Km0
        # Uidt = Vmt * x[4] / (Kmt + x[4])   # old Uidt
        
        # Added Breton model C: ------------------------------------------------------------------
        
        # f(y):
        fy = (x[-2]/(bparams.a*HRb))**bparams.n / (1 + (x[-2]/(bparams.a*HRb))**bparams.n)
        
        # W(t): # updating the w value
        W.update(t, HR, HRb, t_start, t_end, bparams.tz)
        
        # dydt:
        dxdt[-2] = -(1/bparams.THR) * (x[-2] -(HR - HRb))
        
        # dzdt:
        dxdt[-1] = -[fy/bparams.Tin + 1/bparams.Tex] * x[-1] + fy
        
        # Insulin-dependent utilization: 
        Uidt_numerator = params.Vm0*(1 + bparams.beta*x[-2]) + params.Vmx*(1 + bparams.alpha*x[-1]) * (x[6] + params.Ib) - params.Vmx*params.Ib
        Uidt_denominator = Kmt*(1 - bparams.gamma * x[-1] * W() * (x[6] + params.Ib)) + x[4]
    ...
    def reset(self):
        ...
        # Adding y0 and z0 to the init_state array:
        ...
        print("init_state", self.init_state, type(self.init_state), len(self.init_state))
