...
from my_t1dpatient_PA import Action
...
class T1DSimEnv(object):                                 # _Temporary parm_        # _Temporary parm_
    ...
    def mini_step(self, action):
        ...
        ctrl_action_kGI = action.GI/100 * start_statment
        patient_action_kGI = patient_action.GI/100
        
        # Defining the different actions:
        patient_scen_act = Action(insulin=insulin, CHO=patient_action.meal, kGI=patient_action_kGI)
        patient_ctrl_act = Action(insulin=insulin, CHO=ctrl_action_meal, kGI=ctrl_action_kGI)
        
        # PA model variables:
        uhr = self.scenario.get_uhr(self.time)
        ...
        GI = round(patient_action.GI * patient_action.meal/(CHO+1e-6) + ctrl_action_kGI*100 * ctrl_action_meal/(CHO+1e-6))
        
        # next observation
        BG = self.patient.observation.Gsub
        CGM = self.sensor.measure(self.patient)
        
        return CHO, GI, insulin, BG, CGM, uhr
    ...
    def step(self, action, reward_fun=risk_diff):
        ...
        for _ in range(int(self.sample_time)):
            ...
            tmp_CHO, tmp_GI, tmp_insulin, tmp_BG, tmp_CGM, tmp_uhr = self.mini_step(action)
            CHO += tmp_CHO / self.sample_time
            GI += tmp_GI * tmp_CHO / self.sample_time
            insulin += tmp_insulin / self.sample_time
            BG += tmp_BG / self.sample_time
            CGM += tmp_CGM / self.sample_time
            uhr += tmp_uhr / self.sample_time
        GI = round(GI / (CHO+1e-6))
